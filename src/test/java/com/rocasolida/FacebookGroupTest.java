...
public class FacebookGroupTest {
	...
	public void MunicipioNecocheaQuejas() throws Exception {
		...
		try {
			...
			assertTrue("17 publicaciones o mas",page.getPublications().size()>=17);
	...
	public void Elecciones2019SinFraude() throws Exception {
		...
		try {
			// Paso 1) Extraigo las publicaciones[ID,URL y UTIME] de la página principal, en base a una cantidad.
			// ACtualizar LIKES y ver por qué repite un mensaje 4 veceS?
			Group page = fg.scrapeGroup("1172891786215164", minPostUtime, maxPostUtime, null, null, cantComments, CommentsSort.RELEVANCE, null);
			fg.quit();
			System.out.println("group: " + page.toString());
	...
	public void FernandezFernandez() throws Exception {
		...
		try {
			// Paso 1) Extraigo las publicaciones[ID,URL y UTIME] de la página principal, en base a una cantidad.
			// ACtualizar LIKES y ver por qué repite un mensaje 4 veceS?
			Group page = fg.scrapeGroup("336716130365314", minPostUtime, maxPostUtime, null, null, cantComments, CommentsSort.RELEVANCE, null);
